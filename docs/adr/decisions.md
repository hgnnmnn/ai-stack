...
### Update (2026-09-12): q8_0 KV reinstated on both Backends, coder now sized for two concurrent 256k streams

Both `llama-chat` and `llama-coder` had lost their `--cache-type-k/v q8_0` flags at some point — both were running f16 KV, silently. They're back on both. For `llama-coder` (currently `Qwen3.6-35B-A3B`, an MoE/hybrid architecture with `--mmproj` vision active), `--ctx-size`/`--parallel` also moved from `262144`/`1` to `524288`/`2`, i.e. two 256k slots instead of one. The original blocker this ADR opened with — quantized KV combined with multimodal inference on a hybrid architecture being unverified — is back in play for `llama-coder`, which is the one currently pairing vision with quantized KV; `llama-chat` has no `--mmproj` active right now, so the blocker doesn't apply there today, only if it grows a vision projector again. Watch for degraded image understanding on whichever Backend currently pairs vision with quantized KV.
